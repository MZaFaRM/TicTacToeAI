...
def check_winner(board, player):
    ...
    for value in player_value:
        for key, value_ in wins.items():
            if value in key:
                # If player position is part of an item in win position increment by one
                value_ += 1

    return next((player for value__ in wins.values() if value__ == 3), None)
...
def minimax_helper(board):
    ...
    if not _choices or terminal(board):
...
def minimax(board):
    ...
    # Occurs when passed games that already ended
        ...
        raise ValueError("Game already ended.") from e
